...
class MaterialProperties:
    ...
    def __init__(self):
        ...
        self.plastic = {
            'name': 'Plástico de Prolipopileno', 
            'k': 0.22,      # Conductividad térmica [W/m·K]
        ...
        self.microbial_state = {
            ...
            'stress_level': 0.0,
            'activity_factor': 1.0,
            'moisture_stress': 0.0    # Estrés por pérdida de humedad
        }
        
    def _calculate_diffusivities(self):
        """Calcula la difusividad térmica α = k/(ρ·cp) para cada material"""
        self.plastic['alpha'] = self.plastic['k'] / (self.plastic['rho'] * self.plastic['cp'])
    ...
    def create_material_functions(self, domain, material_markers):
        ...
        # Asignar valores: 1 = plástico, 2 = cacao
        k_values = np.where(markers == 1, self.plastic['k'], self.cacao['k'])
        rho_values = np.where(markers == 1, self.plastic['rho'], self.cacao['rho'])
        cp_values = np.where(markers == 1, self.plastic['cp'], self.cacao['cp'])
        ...
        print(f"   Paredes: {self.plastic['name']}")
    ...
    def print_summary(self):
        ...
        print(f"\n{self.plastic['name']} (Paredes):")
        print(f"  - Conductividad (k): {self.plastic['k']:.3f} W/m·K")
        print(f"  - Densidad (ρ): {self.plastic['rho']:.1f} kg/m³")
        print(f"  - Calor específico (cp): {self.plastic['cp']:.1f} J/kg·K")
