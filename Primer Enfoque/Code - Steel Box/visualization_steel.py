...
class BioreactorVisualizer:
    ...
    def plot_2d_evolution(self, stats_file='stats_box_evaporation_steel.json'):
        """Visualización 2D"""
        ...
        # Guardar
        output_file = os.path.join(self.results_dir, 'thermal_evolution_2d_steel.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        print(f"📊 Gráfico 2D guardado: {output_file}")
        
        plt.show()
    
    def visualize_3d_fields(self, xdmf_file='bioreactor_box_evaporation_steel.xdmf', 
                           time_step=-1):
        """
        ...
        # Guardar captura
        output_file = os.path.join(self.results_dir, 'thermal_3d_view_steel.png')
        plotter.screenshot(output_file)
        print(f"📸 Vista 3D guardada: {output_file}")
    
    def create_animation_3d(self, xdmf_file='bioreactor_box_evaporation_steel.xdmf',
                           output_gif='thermal_animation_steel.gif'):
        """
    ...
    def plot_3d_mesh_structure(self):
        ...
        for i in range(grid.n_cells):
            ...
            # Interior (cacao) o exterior (acero)
        ...
        plotter.add_mesh(grid, scalars='Material', 
                        cmap=['white', self.colors['steel'], self.colors['cacao']],
                        show_edges=True, edge_color='gray',
                        opacity=0.8)
    ...
    def create_summary_report(self, stats_file='stats_box_evaporation_steel.json'):
        ...
        # Información clave
        info_text = f"""
        Biorreactor: Caja de acero 85×90×74 cm
        ...
        plt.tight_layout()
        output_file = os.path.join(self.results_dir, 'summary_report_steel.png')
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
