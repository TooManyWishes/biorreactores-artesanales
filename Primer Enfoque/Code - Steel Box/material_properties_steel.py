...
class MaterialProperties:
    ...
    def __init__(self):
        ...
        self.steel = {
            ...
            'k': 16.0,      # Conductividad térmica [W/m·K]
            'rho': 8000.0,    # Densidad [kg/m³]
    ...
    def _calculate_diffusivities(self):
        ...
        self.steel['alpha'] = self.steel['k'] / (self.steel['rho'] * self.steel['cp'])
    ...
    def create_material_functions(self, domain, material_markers):
        ...
        # Asignar valores: 1 = acero, 2 = cacao
        k_values = np.where(markers == 1, self.steel['k'], self.cacao['k'])
        rho_values = np.where(markers == 1, self.steel['rho'], self.cacao['rho'])
        cp_values = np.where(markers == 1, self.steel['cp'], self.cacao['cp'])
        ...
        print(f"   Paredes: {self.steel['name']}")
    ...
    def print_summary(self):
        ...
        print(f"\n{self.steel['name']} (Paredes):")
        print(f"  - Conductividad (k): {self.steel['k']:.3f} W/m·K")
        print(f"  - Densidad (ρ): {self.steel['rho']:.1f} kg/m³")
        print(f"  - Calor específico (cp): {self.steel['cp']:.1f} J/kg·K")
