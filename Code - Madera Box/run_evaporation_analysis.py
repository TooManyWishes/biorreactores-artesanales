...
# Importar el modelo
from main_thermal_model import run_evaporation_simulation
from visualization import BioreactorVisualizer
...
def print_header():
    ...
    print("\n💧 SOLUCIÓN: EVAPORACIÓN NATURAL")
    print("   - ✅ Sin cambios en la geometría de la caja")
    print("   - ✅ Sin ventiladores ni equipos adicionales")
    print("   - ✅ Aprovecha la humedad natural del cacao (40%)")
    print("   - ✅ Convección natural mejorada por gradiente térmico")
    print("\n🎯 EXPECTATIVA:")
    print("   - Sin evaporación: muerte ~53h")
    print("   -: ¿fermentación completa?")
    print("="*70 + "\n")
...
def main():
    ...
    try:
        ...
        if not model.thermal_death_occurred:
            ...
            print(f"   ✅ No se necesitan ventiladores")
        ...
        print("  - Datos 3D: bioreactor_box_evaporation.xdmf")
        print("  - Estadísticas: stats_box_evaporation.json")
        print("  - Reporte: reporte_evaporacion.txt")
...
def create_evaporation_report(model, times, T_max, T_min, T_avg):
    ...
    # Cargar estadísticas
    import json
    stats_file = os.path.join(model.results_dir, f"stats_{model.bioreactor_type}_evaporation.json")
    ...
    # Guardar reporte
    filename = os.path.join(model.results_dir, "reporte_evaporacion.txt")
